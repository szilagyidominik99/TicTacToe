...
public class Game3x3 extends AppCompatActivity implements View.OnClickListener{
    ...
    private ImageButton[] imgBtns = new ImageButton[9];
    private int size = 3;
    private ImageButton[] haveToBlinking = new ImageButton[3];
    private int[][] buttonsValue = {{0,0,0}, {0,0,0}, {0,0,0}};
    ...
    public void onClick(View v) {
        ...
        if(checkIsWinnerState.isWinnerState(row,column,buttonsValue, step , size)){
            ...
            for(int i = 0; i<blinkingButtonId.length; i++){
                int n = getResources().getIdentifier(blinkingButtonId[i], "id", getPackageName());
                haveToBlinking[i] = findViewById(n);
            }
